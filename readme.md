...
* **Permisos Granulares por Token:** Solicitud explícita del scope `calendar.readonly` para que los gestores de proyectos (PMs) puedan leer el estado del calendario.
...
### 📊 3. Motor de Cuotas Dinámicas por Sello (Business Logic)
...
* **Límites de Uso Asimétricos:** Configuración asimétrica de cuotas rígidas dependiendo de la empresa de management o discográfica.

### 🎚️ 4. UI/UX Skeuomórfica de Alto Rendimiento
...
[ Solicitud Normal ] ◄── [ Validador de Reglas de Negocio (GAS) ] ◄─┘
         ...
         └─── (Horas Extra / Fines de Semana) ─────────► [ Transición a Excepción ]
                                                   ...
                                                   [ ScriptProperties (UUID) ]
                                                                 │
                                                                 ▼
                                            [ Alerta Email a Administradores ]
                                                                 │
                                                                 ▼
                                                       [ approval.html ]
                                                                 │
...
* GmailApp API (Sistema de notificaciones y Webhooks).

## 📦 Instrucciones de Despliegue (Setup)

*Nota: Asegúrese de modificar los valores ficticios por los IDs reales de su organización.*

1. **Configurar el Script en Google Apps Script:** Crea un nuevo proyecto en GAS, pega el contenido de `backend_script.js` (renombrado a `.gs`) y configura la variable `CALENDAR_ID`.
2. **Crear el Consentimiento en Google Cloud:** Crea un proyecto en GCP, configura la pantalla de consentimiento OAuth 2.0 y genera un Client ID para aplicaciones web.
3. **Publicar la API:** Implementa el script en GAS como "Aplicación Web" accesible por "Cualquier persona" (para habilitar CORS). Copia la URL terminada en `/exec`.
4. **Configurar Frontend:** Inserta tu `GOOGLE_CLIENT_ID` y tu `GAS_WEBAPP_URL` en los archivos `script.js`, `modificar.js` y `approval.js`. Sube la carpeta a tu servidor web (GitHub Pages, Vercel, Hostinger).
